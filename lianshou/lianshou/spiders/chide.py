...
class ChideSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for name in names:  # 遍历一级菜单名
            ...
            # item = ChiDe_Good_NavItem(cate_name=name_p, lelvel='1', pid=0)  # 存入一级菜单名
            # yield item
            self.count += 1
            a = self.count
            num = 1
            for name1 in names_list:
                ...
                for url in box_list:
                    ...
                    url4 = url3.split("f")[1]
                    class_url = url4
                    # item = ChiDe_Good_NavItem(sub_name=name_s, evel='2', pid=a, cate_id=class_url)
                    # yield item
                    self.count += 1  # 3 4 5
                    r = scrapy.Request(url=url, callback=self.parse_url, dont_filter=True)
                    yield r
                    # item = ChiDe_Good_NavItem(sub_name=name_s, level='2', pid=a, cate_id=class_url)
                    # yield item
                    # item = ChiDe_GoodsItem(cate_id=class_url)
                    # yield item
                    '''
                    获取分类表信息
                    '''
    ...
    def parse_details(self, response):
        ...
        if img_Big:
            ...
            url_str = url.split('-')
            url_str1 = url_str[1].split('.')
            img_item = ChiDe_goods_imageItem(img_small=img_small, img_Big=img_Bigs, datail_url=url_str1[0])
            yield img_item
        else:
            ...
            if imgs_Big:
                ...
                url_str = url.split('-')
                url_str1 = url_str[1].split('.')
                img_item = ChiDe_goods_imageItem(img_small=img_small, img_Big=img_Big_str, datail_url=url_str1[0])
                yield img_item
            else:
                img_big = response.xpath("//div[@id='tabCot_0']/table//img/@src").extract()
                img_Big_str = ','.join(img_big)
                img_smalls = response.xpath("//div[@class='list_img']//a/@href").extract()
                img_small = ','.join(img_smalls)
                url = response.url
                url_str = url.split('-')
